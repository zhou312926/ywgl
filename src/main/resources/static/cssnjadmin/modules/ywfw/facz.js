layui.extend({
    ...
    table.render({
        elem: '#tbl_facz'
        , url: contextPath + '/ywfw/facz/loadList'
        , method: 'post'
        , request: {
            pageName: 'page'
            , limitName: 'limit'
    ...
    table.on('toolbar(tbl_facz)', function (obj) {
        if (obj.event == "refresh") {
            table.reload('tbl_facz');
        }
    });

    table.on('tool(tbl_facz)', function (obj) {
        if (obj.event === 'read') {
